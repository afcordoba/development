...
<%
	...
	Ztmminspeccion inspeccion = (Ztmminspeccion) session.getAttribute("INSPECCION" + session.getId());
	
	String diaInsp = inspeccion.getFecha().substring(8);
	String mesInsp = inspeccion.getFecha().substring(5,7);
	String anioInsp = inspeccion.getFecha().substring(0,4);
	String fechaRV = diaInsp + "-" + mesInsp + "-" + anioInsp;
...
<title>Edición de registro visita <%=tipoDeVisita %></title>
<style type="text/css">
...
<script>
	...
	function habilitarCampos(){
		document.registroVisitasGenerico.rendimiento.removeAttribute("disabled");
		document.registroVisitasGenerico.probabilidad.removeAttribute("disabled");
		document.registroVisitasGenerico.motivos.removeAttribute("disabled");
		document.registroVisitasGenerico.estados.removeAttribute("disabled");
		document.registroVisitasGenerico.estadoFenValorR.removeAttribute("disabled");
		document.registroVisitasGenerico.estadoFenValorV.removeAttribute("disabled");
		document.registroVisitasGenerico.fecha.removeAttribute("disabled");
		document.registroVisitasGenerico.submitBtn.removeAttribute("disabled");		
				            ...
				            (testDate.getFullYear() != iYear))
				 {
				 	...
				 	return false;
						...
						/* EN CASO DE QUERER FILTRAR FECHAS PASADAS COMO INVALIDAS!
							...
							if(testDate.getTime() < new Date(<%=year%>, <%=mes%> - 1, <%=dia%>).getTime()){
...
window.onload=function(){
    	var optionsV = document.registroVisitasGenerico.estadoFenValorV.options;
    	var optionsR = document.registroVisitasGenerico.estadoFenValorR.options;
    	var optionsProbabilidad = document.registroVisitasGenerico.probabilidad.options;
    	var optionsMotivos = document.registroVisitasGenerico.motivos.options;
    	var optionsEstados = document.registroVisitasGenerico.estados.options;
    	var optionsRendimiento = document.registroVisitasGenerico.rendimiento.options;

    	document.registroVisitasGenerico.fecha.value = "<%= fechaRV %>";
    	
    	for(var i=0; i < optionsV.length; i++){
    		if (optionsV[i].value == "<%= inspeccion.getV() %>"){
    			optionsV[i].selected = true;
                break;	
    		}
    	}
    	
    	for(var i=0; i < optionsR.length; i++){
    		if (optionsR[i].value == "<%= inspeccion.getR() %>"){
    			optionsR[i].selected = true;
                break;	
    		}
    	}
    	
    	for(var i=0; i < optionsProbabilidad.length; i++){
    		if (optionsProbabilidad[i].value == "<%= inspeccion.getProbabilidad() %>"){
    			optionsProbabilidad[i].selected = true;
                break;	
    		}
    	}    	
    	
    	for(var i=0; i < optionsMotivos.length; i++){
    		if (optionsMotivos[i].value == "<%= inspeccion.getMotivos() %>"){
    			optionsMotivos[i].selected = true;
                break;	
    		}
    	}    	
    	
    	
    	for(var i=0; i < optionsEstados.length; i++){
    		if (optionsEstados[i].value == "<%= inspeccion.getEstados() %>"){
    			optionsEstados[i].selected = true;
                break;	
    		}
    	}    	
    		

    	for(var i=0; i < optionsRendimiento.length; i++){
    		if (optionsRendimiento[i].value == "<%= inspeccion.getRendEsperado() %>"){
    			optionsRendimiento[i].selected = true;
                break;	
    		}
    	}      	
    	

/* 		document.registroVisitasGenerico.rendimiento.disabled = "disabled";
		document.registroVisitasGenerico.probabilidad.disabled = "disabled";
		document.registroVisitasGenerico.motivos.disabled = "disabled";
		document.registroVisitasGenerico.estados.disabled = "disabled";
		document.registroVisitasGenerico.estadoFenValorR.disabled = "disabled";
		document.registroVisitasGenerico.estadoFenValorV.disabled = "disabled";
		document.registroVisitasGenerico.fechaRV.disabled = "disabled";
		document.registroVisitasGenerico.submitBtn.disabled = "disabled";
		document.registroVisitasGenerico.fechaRV.focus(); */
    };
</script>
...
<table style="border-collapse: collapse" bordercolor="#C0C0C0"
	...
	<tr>
		...
		<table border="0" width="100%">
			<tr>
				<td colspan="3" bgcolor="#006699"><span class="Estilo2">Edición registro de visita <%=tipoDeVisita %></span></td>
			</tr>
			<tr>
				<td>
				<hr>
			...
			<tr>
				<td width="100%" colspan="3">
					<input type="text" name="fecha" value="" disabled="disabled">
				</td>
			</tr>
			<tr>
				<td colspan="3">
			...
			<tr>
				...
				<div>
        				<select name="estadoFenValorV" size="1" disabled="disabled">
			...
			<tr>
				<td width="10%" colspan="3">
					<div>
	        				<select name="estadoFenValorR" size="1" disabled="disabled">
								<option value="">-- Elegir --</option>
			...
			<tr>
				<td width="100%" colspan="2">
						<select name="estados" size="1" disabled="disabled">
							<option value="">-- Elegir --</option>
			...
			<tr>
				<td width="100%" colspan="3">
						<select name="motivos" size="1" disabled="disabled">
          					<option value="">-- Elegir --</option>
			...
			<tr>
				<td width="100%" colspan="1">
						<select name="probabilidad" size="1" disabled="disabled">
							<option value="">-- Elegir --</option>
			...
			<tr>
				<td width="100%" colspan="1">
						<select name="rendimiento" size="1" disabled="disabled">
							<option value="">-- Elegir --</option>
			...
			<tr><td width="100%" colspan="1"><input class="boton"  type="button" onclick="habilitarCampos()" value="Editar"/><br></td></tr>			
			<tr>
				<td width="100%" colspan="1"><input class="boton" type="submit" value="Cargar" id="submitBtn" disabled="disabled"><br>
				</td>
			</tr>
			<tr><td width="100%" colspan="1"><input class="boton"  type="button" onclick="javascript:window.location.href='<%=context%>/ServiceSap?accion=RegistroDeVisitasCliente&asig=<%=planificacion.getIdAsignacion()%>&asigPos=<%=planificacion.getIdAsignPos()%>&plaPos=<%=planificacion.getIdPlanificPos()%>';" value="Cancelar"/><br></td></tr>        
			<tr><td width="100%" colspan="1"><font color="red"></font></td></tr>
							
		</table>
		<input type="hidden" name="accion" value="modificacionRegistroVisitasGenerico">
