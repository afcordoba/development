...
public class DaoScada {
	...
	private void closeConnection() throws SQLException {
			if(rs != null){
				rs.close();
				rs = null;
			}else new ConnectionErrorException();
			if (con != null){
				con.close();
				con = null;
			}else new ConnectionErrorException();
	...
	public List<Camara> getInformacionDeCamara(String nombreTabla, String fecha) throws ClassNotFoundException, SQLException {
		...
		con = this.getConnection();
		String query = "SELECT  VarName,TimeString ,VarValue from " + nombreTabla + " where Cast(TimeString as datetime) >= Cast('"+ fecha + "' as datetime)" ;
		con = this.getConnection();
		if (con != null) {
			ps = null;
			rs = null;
